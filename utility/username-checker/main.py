...
Logger.log(f"Username-checker started at {time()}")
...
SIXTEENCHECKER = Pomelo()
Logger.log("username-checker successfully initiated")
...
def WEBHOOK_PROCESSOR():
    ...
    while True:
        ...
        if len(names_diff) > 1 and last_send_time-time() < 5: #if we get hits frequently and user is not scaning for super rare names we switch to batch mode
            ...
            json = {"content": "\n".join(payload), 'username': 'e-z.bio/0.o', 'avatar_url': 'https://yt3.googleusercontent.com/er_4m6es8kZug1kut1x4893xWStytBbvFPtiq2Wy0d7rnHcZ0t4fR-GRjYv6nb_jidz1EHDT=s160-c-k-c0x00ffffff-no-rj'}
            
            x = SIXTEENCHECKER.session.post(
                url=webhook,
                json=json
            )
            ...
            json = {"content": message.replace("<name>", name).replace("<time>", str(hittime)).replace("<elapsed>", str(round(current_time - start_time))).replace("<RPS>", str(RPS)), 'username': 'e-z.bio/0.o', 'avatar_url': 'https://yt3.googleusercontent.com/er_4m6es8kZug1kut1x4893xWStytBbvFPtiq2Wy0d7rnHcZ0t4fR-GRjYv6nb_jidz1EHDT=s160-c-k-c0x00ffffff-no-rj'}
            x = SIXTEENCHECKER.session.post(
                url=webhook,
                json=json
            )
